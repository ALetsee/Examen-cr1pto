...
def flujo_cifrar():
    ...
    # Cifrado paso a paso
    print(AMARILLO + "\n--- Cifrado ECC (ElGamal) por caracter ---" + RESET)
    bloques = []
    for c in mensaje:
...
def flujo_grafica():
    ...
    for x in range(primo):
        ...
        for y in range(primo):
            if (y * y) % primo == rhs:
                puntos_curva_x.append(x)
                puntos_curva_y.append(y)

    # Puntos del mensaje
    puntos_msg = [(ascii_a_punto[ord(c)], c) for c in mensaje]

    # Graficar
    fig, ax = plt.subplots(figsize=(9, 7))
    ax.scatter(puntos_curva_x, puntos_curva_y, s=1, color='steelblue', alpha=0.5, label='Puntos de la curva')

    # Punto G
    ax.scatter(punto_base[0], punto_base[1], s=60, color='orange', zorder=5, label=f'G = {punto_base}')

    # Puntos del mensaje
    colores = plt.cm.Set1.colors
    vistos = {}
    for i, ((px, py), c) in enumerate(puntos_msg):
        color = colores[i % len(colores)]
        etiqueta = f"'{c}' ({ord(c)}*G)" if c not in vistos else None
        vistos[c] = True
        ax.scatter(px, py, s=80, color=color, zorder=6, label=etiqueta)
        ax.annotate(f"'{c}'", (px, py), textcoords="offset points",
                    xytext=(6, 4), fontsize=8, color=color)

    ax.set_title(f"Curva eliptica: y² = x³ - x + {coef_b}  (mod {primo})", fontsize=13)
    ax.set_xlabel("x")
    ax.set_ylabel("y")
    ax.legend(loc='upper right', fontsize=7, markerscale=2)
    ax.set_xlim(-5, primo + 5)
    ax.set_ylim(-5, primo + 5)
    plt.tight_layout()

    print(AMARILLO + "\n--- Por que la grafica se ve asi ---" + RESET)
    print(f"  La curva opera en mod {primo}, por lo que solo existen")
    print(f"  coordenadas enteras entre 0 y {primo-1}. De ahi los puntos discretos.")
    print(f"  Cada punto (x, y) tiene su simetrico (x, {primo}-y) en la curva.")
    print(f"  La grafica tiene {len(puntos_curva_x)} puntos en total.")
    print()
    print(f"  Puntos del mensaje '{mensaje}':")
    for (px, py), c in puntos_msg:
        print(f"    '{c}' (ASCII {ord(c)}) -> {ord(c)}*G = ({px}, {py})")
    print(f"  Cada letra se convierte a ASCII y se multiplica por G.")
    print(f"  El mod {primo} dispersa el resultado por toda la grafica.")
...
# MAIN
OPCIONES = ["Cifrar mensaje", "Descifrar mensaje", "Ver grafica de la curva", "Salir"]
