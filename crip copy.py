...
def flujo_cifrar():
    ...
    for c in mensaje:
        ...
        print(f"  '{c}' -> C1: {C1[0]},{C1[1]}   C2: {C2[0]},{C2[1]}")
...
def flujo_descifrar():
    ...
    print(AMARILLO + "\nIngresa C1 y C2 por separado para cada caracter." + RESET)
    print("Linea vacia en C1 para terminar.\n")
    ...
    while True:
        try:
            entrada_c1 = input(f"Bloque {idx} - C1 (x,y): ").strip()
        except EOFError:
            break
        if not entrada_c1:
            break
        ...
        try:
            entrada_c2 = input(f"Bloque {idx} - C2 (x,y): ").strip()
        except EOFError:
            break

        try:
            p1 = [int(v.strip()) for v in entrada_c1.split(',')]
            p2 = [int(v.strip()) for v in entrada_c2.split(',')]
            if len(p1) != 2 or len(p2) != 2:
                raise ValueError("Cada bloque requiere exactamente x,y")
            C1 = (p1[0], p1[1])
            C2 = (p2[0], p2[1])
        except ValueError as e:
            print(ROJO + f"Error de formato: {e}" + RESET)
            continue
